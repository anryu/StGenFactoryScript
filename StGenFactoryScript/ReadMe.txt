================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: StGenFactoryScript プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した StGenFactoryScript には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには StGenFactoryScript アプリケーションを構成している各ファイ
ルの概要説明が含まれます。

StGenFactoryScript.vcxproj
...
StGenFactoryScript.vcxproj.filters
...
StGenFactoryScript.h
   これはアプリケーションのメイン ヘッダー ファイルです。このファイルには、
   Resource.h を含む、その他のプロジェクト固有のヘッダーが含まれていて、
   CStGenFactoryScriptApp アプリケーション クラスを宣言します。

StGenFactoryScript.cpp
   これは、アプリケーション クラス CStGenFactoryScriptApp を含むメイン アプリケー
   ション ソース ファイルです。

StGenFactoryScript.rc
...
res\StGenFactoryScript.ico
   これは、アプリケーションのアイコンとして使用されるアイコンファイルです。この
   アイコンはメイン リソース ファイル StGenFactoryScript.rc に含まれていま
   す。

res\StGenFactoryScript.rc2
...
StGenFactoryScriptDlg.h, StGenFactoryScriptDlg.cpp - ダイアログ
   これらのファイルは CStGenFactoryScriptDlg クラスを含みます。このクラスはアプ
   リケーションのメイン ダイアログの動作を定義します。ダイアログ テンプレートは
   Microsoft Visual C++ で編集可能な StGenFactoryScript.rc に含まれます。
...
StdAfx.h, StdAfx.cpp
   ...
   StGenFactoryScript.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
